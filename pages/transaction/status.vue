              ...
              >
                ...
                <template v-slot:item.status="{ item }">
                  <a
                    class="bni-primary"
                    @click.prevent="
                      showTransactionStatusDetail(
                        item.status != 'Done' ? item.errorMessage : ''
                      )
                    "
                  >
                    <b
                ...
                <template v-slot:item.name="{ item }">
                  <a @click.prevent="showDetail(item)" class="bni-primary">
                    {{ item.name }}
                  </a>
                </template>
              </v-data-table>
            </template>
            <div class="row pt-2 d-flex" style="justify-content: space-between">
      ...
      <v-dialog v-model="uploadDialog">
        <v-card>
          ...
          >
            ...
            <ValidationMessage
              v-if="index == form.documents.length - 1"
    ...
    validate: async function () {
      ...
      for (let i = 0; i < this.form.documents.length; i++) {
        ...
        if (!this.form.documents[i].attachment.name) {
          this.validationMessage.push("Upload file ke-" + (i + 1));
        }
      }
    },
    async submit() {
